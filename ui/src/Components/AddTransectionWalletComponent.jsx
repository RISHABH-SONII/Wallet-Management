...
export default function AddTransectionWalletComponent() {
  ...
  let [transectionForm, setTransectionForm] = useState(true); // To open or close the transection model
  ...
  let handleTransectionModel = () => {
    setTransectionForm(!transectionForm);
  };
  ...
  let handleAddTransection = (event) => {
    let currentTransectionFormData = {
    ...
    try {
      axios
        .post(
          "https://localhost:7242/api/Users/addTransection",
          currentTransectionFormData
        )
        .then((myresponse) => myresponse.data)
        .then((finalresponse4) => {
          console.log(finalresponse4);
  ...
  return (
    <Modal open={transectionForm} onClose={handleTransectionModel}>
      <Box
        sx={{
          position: "absolute",
          top: "50%",
